...
static void MPR121_Config_Registers(char* buf)
{
  ...
  for (i = 0; i < 4; i++) {
    MPR121_WriteReg(MPR121_REG_E0TTH + MPR121_Cap_Mouse_Pinmux[i] * 2, CAP_MOUSE_TOU_THRESH);
    MPR121_WriteReg(MPR121_REG_E0RTH + MPR121_Cap_Mouse_Pinmux[i] * 2, CAP_MOUSE_REL_THRESH);
  }
  for (i = 0; i < 7; i++) {
    MPR121_WriteReg(MPR121_REG_E0TTH + MPR121_TouchBar_Pinmux[i] * 2, TOUCHBAR_TOU_THRESH);
    MPR121_WriteReg(MPR121_REG_E0RTH + MPR121_TouchBar_Pinmux[i] * 2, TOUCHBAR_REL_THRESH);
  }
...
* Return         : None
*******************************************************************************/
static uint16_t MPR121_ELE_to_Pin(uint16_t ele_data, uint8_t* pinmux)
{
  uint8_t i;
  uint16_t pin_data = 0;
  for (i = 0; i < sizeof(MPR121_TouchBar_Pinmux); i++) {
    pin_data |= (ele_data & 1) << pinmux[i];
    ele_data >>= 1;
  }
  return pin_data;
}
...
void FLASH_Read_MPR121_ALG_Parameter(void)
{
  ...
  if (mpr_algParameter.magic != ALG_PARAM_MAGIC) {
    ...
    mpr_algParameter.cap_mouse_event_thr = 0xA;
  }
}
...
void MPR121_ALG_Judge_Cap_Mouse(void)
{
  ...
  uint16_t sum1, sum2;
  uint8_t ret;
  uint16_t now_dat = MPR121_ELE_to_Pin(cap_mouse_sts_head[0]->dat, MPR121_Cap_Mouse_Pinmux);
  uint16_t pre_dat = MPR121_ELE_to_Pin(cap_mouse_sts_head[0]->prev->dat, MPR121_Cap_Mouse_Pinmux);
  ...
  if (sum1 < sum2 && (sum2 - sum1) > mpr_algParameter.cap_mouse_event_thr) YMovement--;
  else if (sum1 > sum2 && (sum1 - sum2) > mpr_algParameter.cap_mouse_event_thr) YMovement++;
  /* judge down */
  ...
  if (sum1 < sum2 && (sum2 - sum1) > mpr_algParameter.cap_mouse_event_thr) XMovement++;
  else if (sum1 > sum2 && (sum1 - sum2) > mpr_algParameter.cap_mouse_event_thr) XMovement--;
  /* judge right */
...
void MPR121_ALG_Judge_Touchbar(void)
{
  ...
  uint8_t record;
  uint16_t now_dat = MPR121_ELE_to_Pin(touchbar_head[0]->dat, MPR121_TouchBar_Pinmux);
  uint16_t pre_dat = MPR121_ELE_to_Pin(touchbar_head[0]->prev->dat, MPR121_TouchBar_Pinmux);
